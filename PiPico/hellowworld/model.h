#ifndef TENSORFLOW_LITE_MICRO_EXAMPLES_MAINTENANCE_MODEL_H_
#define TENSORFLOW_LITE_MICRO_EXAMPLES_MAINTENANCE_MODEL_H_

alignas(8) const unsigned char g_model[] = {
  
};
unsigned int g_model_len = 2024;


#endif  // TENSORFLOW_LITE_MICRO_EXAMPLES_MAINTENANCE_MODEL_H_
