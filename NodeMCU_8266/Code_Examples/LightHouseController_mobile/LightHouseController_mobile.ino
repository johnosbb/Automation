...
void MQTTCallback(char* topic, byte* payload, unsigned int length) {
  ...
  Serial.println();
  if(strcmp("stat/LIGHTHOUSE/MAIN_LAMP_ON", topic) == 0)
  {
      Serial.print("Turning on main lamp");
      turn_on(main_leds,NUM_LEDS_MAIN_LAMP,&mainLampColourReference);
  }
  else if(strcmp("stat/LIGHTHOUSE/MAIN_LAMP_OFF",topic)== 0)
  {
      turn_off(main_leds,NUM_LEDS_MAIN_LAMP);
  }
  else if(strcmp("stat/LIGHTHOUSE/ALL_LAMPS_ON", topic) == 0)
  {
      Serial.print("Turning on all lamps");
      turn_on_all_lamps();
  }
  else if(strcmp("stat/LIGHTHOUSE/ALL_LAMPS_OFF",topic)== 0)
  {
      turn_off_all_lamps();
  }
}
