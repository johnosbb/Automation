...
void ledOnOff(AutoConnectButton& me, AutoConnectAux& ledOnOff) {
  if (me.value == "ON") {
    ...
    // For example, a following statement changes the font color of the `m_caption`
    // element along with a LED blinking.
    ledOnOff["m_caption"].response("style", "{\"color\":\"red\", \"font-weight\":\"bold\"}");
  }
  if (me.value == "OFF") {
    ...
    me.response("ON");
    ledOnOff["m_caption"].response("style", "{\"color\":\"black\", \"font-weight\":\"normal\"}");
  }
}
...
void setup() {
  ...
  AutoConnectButton& onOff = led["m_onoff"].as<AutoConnectButton>();
  onOff.on(ledOnOff);
